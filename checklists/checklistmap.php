...
<head>
	...
	<script type="text/javascript">
		...
		function initialize(){
			...
			const vIcon = new google.maps.MarkerImage("../images/google/smpin_red.png");
			const pIcon = new google.maps.MarkerImage("../images/google/smpin_blue.png");
			<?php
				$mCnt = 0;
				foreach($coordArr as $tid => $cArr){
					foreach($cArr as $pArr){
						?>
                        const pt = new google.maps.LatLng(<?php echo $pArr['ll']; ?>);
                        llBounds.extend(pt);
						<?php
						if(array_key_exists('occid',$pArr)){
							?>
							const m<?php echo $mCnt; ?> = new google.maps.Marker({position: pt, map:map, title:"<?php echo $pArr['notes']; ?>", icon:vIcon});
							google.maps.event.addListener(m<?php echo $mCnt; ?>, "click", function(){ openIndPU(<?php echo $pArr['occid']; ?>); });
							<?php
						}
						else{
							?>
							const m<?php echo $mCnt; ?> = new google.maps.Marker({position: pt, map:map, title:"<?php echo $pArr['sciname']; ?>", icon:pIcon});
							<?php
						}
						$mCnt++;
					}
            ...
            var polyPt = new google.maps.LatLng(<?php echo $llArr[0].','.$llArr[1]; ?>);
					...
					?>
			const footPoly = new google.maps.Polygon({
				paths: polyPointArr,
				strokeWeight: 2,
				fillOpacity: 0.4,
				map: map
		...
		function openIndPU(occId){
			const puWin = window.open('../collections/individual/index.php?occid='+occId,'indspec' + occId,'scrollbars=1,toolbar=0,resizable=1,width=900,height=600,left=20,top=20');
