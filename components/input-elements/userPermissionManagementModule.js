const userPermissionManagementModule = {
    ...
    template: `
        <div class="q-pa-md column q-col-gutter-sm">
            ...
            <q-card flat bordered>
                <q-card-section>
                    <div class="text-h6 text-bold">Add a new {{ permissionLabel }}</div>
                    <div class="row justify-between q-gutter-sm no-wrap">
                        ...
                        <div class="col-2 row justify-end">
                            <div>
                                <q-btn color="secondary" @click="addUser();" label="Add User" :disabled="!selectedAddUser" />
                            </div>
                        </div>
                    </div>
                </q-card-section>
    ...
    setup(props, context) {
        ...
        function addUser() {
            ...
            formData.append('uid', selectedAddUser.value['uid'].toString());
        ...
        function processAddUserChange(user) {
            const existingObj = permittedUserArr.value.find(puser => Number(puser['uid']) === Number(user['uid']));
            if(existingObj){
                showNotification('negative', 'That user already is included in the user list');
            }
            else{
                selectedAddUser.value = user;
            }
        }
