const singleLanguageAutoComplete = {
    ...
    template: `
        <q-select v-model="language" use-input fill-input hide-selected outlined dense options-dense hide-dropdown-icon use-input popup-content-class="z-top" behavior="menu" input-debounce="0" bg-color="white" @new-value="createValue" :options="autocompleteOptions" option-value="iso" option-label="name" @filter="getOptions" @blur="blurAction" @update:model-value="processChange" :label="label" :tabindex="tabindex" :disable="disabled">
