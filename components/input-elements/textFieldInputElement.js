const textFieldInputElement = {
    ...
    template: `
        <template v-if="fieldHint">
            ...
            <template v-else>
                <q-input outlined v-model="value" :type="inputType" :step="step" :label="label" :debounce="debounce" bg-color="white" @update:model-value="processValueChange" @keyup.enter="processEnterClick" :readonly="disabled" :autogrow="inputType === 'textarea'" :hint="fieldHint" :tabindex="tabindex" :name="field" autocomplete="on" dense>
        ...
        <template v-else>
            ...
            <template v-else>
                <q-input outlined v-model="value" :type="inputType" :step="step" :label="label" :debounce="debounce" bg-color="white" @update:model-value="processValueChange" @keyup.enter="processEnterClick" :readonly="disabled" :autogrow="inputType === 'textarea'" :tabindex="tabindex" :name="field" autocomplete="on" dense>
                    <template v-if="!disabled && ((value && clearable) || definition)" v-slot:append>
                        <q-icon role="button" v-if="definition" name="help" class="cursor-pointer" @click="openDefinitionPopup();" @keyup.enter="openDefinitionPopup();" aria-label="See field definition" :tabindex="tabindex">
                            <q-tooltip anchor="top middle" self="bottom middle" class="text-body2" :delay="1000" :offset="[10, 10]">
                                See field definition
