    ...
    template: `
        ...
        <q-dialog v-model="rankSelectDialog">
            <q-card>
                ...
                <q-card-section>
                    <div>
                        <q-checkbox indeterminate-value="some" v-model="selectAll" label="Select All" @update:model-value="selectAllChange" />
                    </div>
                    <q-separator size="1px" color="grey-8" class="q-ma-md"></q-separator>
                    <template v-for="option in rankOptions">
                        <q-checkbox v-model="selectedRanks" :val="option.rankid" :label="option.rankname" @update:model-value="processChange" />
                    </template>
                </q-card-section>
            </q-card>
        </q-dialog>
