const cogeUrl = "https://coge.geo-locate.org/symbiota/";
let t;
let t2;
const datasetList = {};
...
function cogeUpdateCount(formObj){
	...
	$.ajax({
		...
		if(response === 0) {
			f.builddwcabutton.disalbed = true;
		}
...
function cogeSubmitData(dwcaPath){
	...
	$.ajax({
		...
		const result = response.result;
		const dataSourceGuid = result.datasourceId;
		if(dataSourceGuid){
			$("#coge-push2coge").hide();
...
function cogeCheckStatus(id){
	$.ajax({
		...
		if(result === "authentication required"){
			...
			clearInterval(t2);
			const iStatus = result.importStatus.state;
		...
		if(result === "authentication required"){
			...
			for(const i in result){
				if(result.hasOwnProperty(i)){
					...
					let subText = 'title="You are not able to submit datasets with User level permissions. Contact the GeoLocate project administrator to adjust your permissions" disabled';
					...
					if(dataSources){
						htmlOut = htmlOut + '<fieldset style="margin:0 30px;padding:10px"><legend><b>Datasets</b></legend>';
						datasetList[name] = {};
						for(const j in dataSources){
							if(dataSources.hasOwnProperty(j)){
								datasetList[name][j] = dataSources[j].name;
								htmlOut = htmlOut + "<div><b>" + dataSources[j].name + "</b> (";

								const uploadType = dataSources[j].uploadType;
								if(uploadType === "csv"){
								...
								if(uploadedBy) {
									htmlOut = htmlOut + "; " + uploadedBy;
								}

								htmlOut = htmlOut + ")";
								const dsDescr = dataSources[j].description;
								if(dsDescr) {
									htmlOut = htmlOut + ": " + dsDescr;
								}
								htmlOut = htmlOut + "</div>";
								if(uploadType === "Symbiota (DwCA)"){
			...
			if(htmlOut === "") {
				htmlOut = "<div>There appears to be no projects currently associated with your GeoLocate user profile</div>";
			}
...
function cogeCheckGeorefStatus(id){
	$.ajax({
		...
		if(result === "authentication required"){
			...
			const specStats = result.stats.specimens;
			const localStats = result.stats.localities;
			let htmlOut = '<div style="border:1px solid black">';
...
function verifyDataSourceIdentifier(f){
	const newProjName = $("input[name=cogename]").val();
	if(newProjName !== "" && $('input[name=cogecomm]:checked').length > 0){
		if($('input[name=cogecomm]:checked').val() in datasetList){
			const projList = datasetList[$('input[name=cogecomm]:checked').val()];
			for(const h in projList){
				if(projList.hasOwnProperty(h)){
