...
class PluginsManager {
	...
	public function setSlidewhow($ssId,$numSlides,$numDays,$imageType,$clId,$dayInterval){
		...
		$lastCLID = 0;
		$previous = 0;
		if(file_exists($GLOBALS['SERVER_ROOT'].'/temp/slideshow/'.$ssId.'_info.json')){
			$oldArr = json_decode(file_get_contents($GLOBALS['SERVER_ROOT'].'/temp/slideshow/'.$ssId.'_info.json'), true);
		...
		if($replace === 1){
			...
			if($clId){
				$previous = array();
				if(file_exists($GLOBALS['SERVER_ROOT'].'/temp/slideshow/'.$ssId.'_previous.json')){
					$previous = json_decode(file_get_contents($GLOBALS['SERVER_ROOT'].'/temp/slideshow/'.$ssId.'_previous.json'), true);
					unlink($GLOBALS['SERVER_ROOT'].'/temp/slideshow/'.$ssId.'_previous.json');
					if($clId !== $lastCLID){
						$previous = array();
					}
				}
			}
			else if(file_exists($GLOBALS['SERVER_ROOT'].'/temp/slideshow/'.$ssId.'_previous.json')){
				unlink($GLOBALS['SERVER_ROOT'].'/temp/slideshow/'.$ssId.'_previous.json');
			}
			if(file_exists($GLOBALS['SERVER_ROOT'].'/temp/slideshow/'.$ssId.'_info.json')){
				unlink($GLOBALS['SERVER_ROOT'].'/temp/slideshow/'.$ssId.'_info.json');
			}
			...
			if($clId){
				$fp = fopen($GLOBALS['SERVER_ROOT'].'/temp/slideshow/'.$ssId.'_previous.json', 'wb');
				fwrite($fp, json_encode($previous));
				fclose($fp);
			}
			$fp = fopen($GLOBALS['SERVER_ROOT'].'/temp/slideshow/'.$ssId.'_info.json', 'wb');
			fwrite($fp, json_encode($ssIdInfo));
			fclose($fp);
		}

		return json_decode(file_get_contents($GLOBALS['SERVER_ROOT'].'/temp/slideshow/'.$ssId.'_info.json'), true);
	}
	...
	public function getImageList($imageArr,$width): string
	{
		...
		foreach($imageArr as $igmAr => $imgIdArr){
			...
			$linkUrl = '';
			if($imgIdArr['occid']){
				$linkUrl = $GLOBALS['CLIENT_ROOT'].'/collections/individual/index.php?occid='.$imgIdArr['occid'].'&clid=0';
			}
			elseif($imgIdArr['tid']){
				$name = str_replace(' ','%20',$imgIdArr['SciName']);
				$linkUrl = $GLOBALS['CLIENT_ROOT'].'/taxa/index.php?taxon='.$name;
			}
	...
	public function setShowHtml($imageHtml,$width,$interval): string
	{
		...
		$html .= "src:url('".$GLOBALS['CLIENT_ROOT']."/css/images/fontawesome-webfont.eot?v=3.0.1');";
		$html .= "src:url('".$GLOBALS['CLIENT_ROOT']."/css/images/fontawesome-webfont.eot?#iefix&v=3.0.1') format('embedded-opentype'),";
		$html .= "url('".$GLOBALS['CLIENT_ROOT']."/css/images/fontawesome-webfont.woff?v=3.0.1') format('woff'),";
		$html .= "url('".$GLOBALS['CLIENT_ROOT']."/css/images/fontawesome-webfont.ttf?v=3.0.1') format('truetype');";
		...
		$html .= 'background-image: url('.$GLOBALS['CLIENT_ROOT'].'/css/images/btns-next-prev.png);';
		$html .= 'background-repeat: no-repeat;}';
		$html .= '.slidesjs-pagination li a {';
		$html .= 'background-image: url('.$GLOBALS['CLIENT_ROOT'].'/css/images/pagination.png);';
		$html .= 'background-position: 0 0;}';
		$html .= '</style>';
		$html .= '<script src="'.$GLOBALS['CLIENT_ROOT'].'/js/jquery.js"></script>';
		$html .= '<script src="'.$GLOBALS['CLIENT_ROOT'].'/js/jquery.slides.js"></script>';
		$html .= '<script src="'.$GLOBALS['CLIENT_ROOT'].'/js/symb/plugins.js"></script>';
		$html .= $imageHtml;
		$html .= '<script>';
		$html .= '$(function() {';
