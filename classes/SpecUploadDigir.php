...
class SpecUploadDigir extends SpecUploadBase {
	...
	private function submitReq(): void
	{
		$digirEof = false;
		$currentReturn = 0;
		$matchCount = 0;
		...
		do{
			$digirProtocol = 'http://digir.net/schema/protocol/2003/1.0';
			$digirSourceXsd = 'http://digir.sourceforge.net/schema/protocol/2003/1.0/digir.xsd';
			$darwin1SourceXsd = 'http://digir.net/schema/conceptual/darwin/2003/1.0';
			$darwin2SourceXsd = 'http://digir.sourceforge.net/schema/conceptual/darwin/2003/1.0/darwin2.xsd';
			$url = (stripos($this->server, 'http://') !== false? '' : 'http://').$this->server.$this->path. '?doc=' .urlencode('<request ' .
					...
					"xsi:schemaLocation='".$digirProtocol. ' ' .$digirSourceXsd. ' ' .$darwin1SourceXsd. ' ' .$darwin2SourceXsd."'>".
					'<header>' .
					'<version>1.0</version>' .
					'<sendTime>' .date(DATE_ATOM). '</sendTime>' .
		...
		elseif($this->withinRecordElement && $this->activeFieldName && $this->activeFieldValue){
			...
			elseif($this->activeFieldName === 'VERBATIMCOLLECTINGDATE' || $this->activeFieldName === 'VERBATIMEVENTDATE'){
				...
				if(!array_key_exists('EVENTDATE',$this->fieldDataArr)){
					$datetime = strtotime($this->activeFieldValue);
					if($datetime) {
						$this->fieldDataArr['EVENTDATE'] = date('Y-m-d H:i:s', $datetime);
					}
				}
			}
			...
			else if(!array_key_exists($this->activeFieldName,$this->fieldDataArr)){
	...
	public function setSearchStart($start): void
	{
		$this->searchStart = $start;
	}

	public function getSearchStart(): int
	{
		return $this->searchStart;
	}

	public function setSearchLimit($limit): void
	{
